...
if __name__ == '__main__':
    ...
    train_features, train_names = extract_features(model, data_loader_train, args.use_cuda, multiscale=args.multiscale, return_csv = True)
    query_features, query_names = extract_features(model, data_loader_query, args.use_cuda, multiscale=args.multiscale, return_csv = True)
    
    train_features = nn.functional.normalize(train_features, dim=1, p=2)
    query_features = nn.functional.normalize(query_features, dim=1, p=2)
    
    os.makedirs(args.output_dir, exist_ok = True)
    csv_dict = {"namefile": train_names[0], "label": train_names[1]}
    csv_dict = pd.DataFrame.from_dict(csv_dict)
    csv_dict.to_csv(args.output_dir + "train_features.csv", index = False)
    
    csv_dict = {"namefile": query_names[0], "label": query_names[1]}
    csv_dict = pd.DataFrame.from_dict(csv_dict)
